        ...
        <div className="flex items-center gap-6">
           ...
           <div>
              <div className="flex items-center gap-3 mb-1">
                 <h1 className="text-3xl font-black text-slate-900 tracking-tight">{project?.name || 'Unnamed Project'}</h1>
                 <span className="px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border bg-blue-50 text-blue-600 border-blue-100">{project?.status || 'PLANNING'}</span>
              </div>
              <p className="text-sm font-bold text-slate-400 uppercase tracking-tighter">{project?.client || 'Internal'} • {project?.code || 'NO-CODE'}</p>
           </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-12 gap-8">
           <div className="col-span-12 xl:col-span-8 space-y-8">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 ...
                 <KpiCard label="Tasks Done" val={`${tasks?.filter(t => t.status === 'COMPLETED').length || 0} / ${tasks?.length || 0}`} color="emerald" />
           ...
           <div className="col-span-12 xl:col-span-4 space-y-8">
              <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
                 ...
                 <div className="space-y-6">
                    <HealthRow label="Schedule Velocity" val="100%" />
           ...
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {tasks?.map(task => (
                <div key={task._id} className="p-6 bg-slate-50 border border-slate-100 rounded-[2rem] space-y-4">
                   ...
                   <h4 className="font-black text-slate-900 leading-tight">{task?.title || 'Untitled Task'}</h4>
