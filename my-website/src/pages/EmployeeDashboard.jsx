...
const EmployeeDashboard = () => {
  ...
  useEffect(() => {
    const handleUpdate = (e) => {
      setUser(e.detail);
      if (data) setData(prev => prev ? { ...prev, user: e.detail } : null);
    };
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        if (result.success) {
          setData(result.data);
          setUser(result.data.user || getAuthUser());
        }
      } catch (err) {
  ...
  if (loading || !data) {
    return (
      <div className="flex items-center justify-center h-[60vh] font-sans">
    ...
    <div className="animate-in fade-in duration-700 font-sans">
        ...
        <div className="mb-12 flex flex-col xl:flex-row gap-8 items-stretch">
            ...
            <div className="flex-1 bg-white rounded-[2.5rem] p-8 border border-slate-200/60 shadow-sm shadow-blue-500/5 flex flex-col justify-center">
                ...
                <div className="flex flex-wrap gap-3">
                   <span className="px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-[11px] font-black uppercase tracking-widest border border-blue-100">Portal ID: {user?.employeeId || 'RCS-USR'}</span>
                ...
                <p className="mt-6 text-slate-500 font-medium leading-relaxed">
                  You've completed <span className="text-blue-600 font-black">{kpis?.tasksCompleted || 0} tasks</span> recently. <br/>
                  Your current performance rank is <span className="text-amber-500 font-black">{stats?.rank || 'N/A'}</span>. Keep it up!
                </p>
            </div>
            ...
            <div className="w-full xl:w-[450px] bg-slate-900 rounded-[2.5rem] p-8 text-white relative overflow-hidden flex items-center gap-6 shadow-2xl shadow-slate-900/20 border border-white/5">
                ...
                <div className="relative z-10">
                    <h2 className="text-2xl font-black mb-1 truncate max-w-[200px]">{user?.name || 'Authorized Personnel'}</h2>
                    <p className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-4">{user?.designation || 'Staff Member'}</p>
                    <div className="space-y-2">
                       <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                          <Building2 size={12} className="text-slate-500" /> {user?.department || 'RCS Global'}
                       </div>
                       <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
           <MetricCard
             ...
             sub={stats?.todayAttendance ? `Check-in recorded` : 'Action Required'}
           ...
           <MetricCard
             ...
             value={`${stats?.monthlyAttendance?.present || 0} / ${stats?.monthlyAttendance?.total || 24} Days`}
             sub={`${Math.round(((stats?.monthlyAttendance?.present || 0) / (stats?.monthlyAttendance?.total || 24)) * 100)}% Completion Rate`}
             progress={((stats?.monthlyAttendance?.present || 0) / (stats?.monthlyAttendance?.total || 24)) * 100}
           />
           <MetricCard
             icon={Trophy}
             color="amber"
                   ...
                   <div className="relative w-44 h-44 flex items-center justify-center flex-shrink-0">
                      <svg className="w-full h-full transform -rotate-90">
                         ...
                         <circle
                           ...
                           strokeDasharray="477.5" strokeDashoffset={477.5 * (1 - (scoreboard?.score || 0) / 10)}
                           strokeLinecap="round"
                         />
                         <defs>
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
                         <span className="text-4xl font-black text-slate-900 leading-none">{scoreboard?.score || 0}</span>
                         <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Score</span>
                      </div>
                   </div>

                   <div className="flex-1 w-full space-y-6">
                      <ScoreBar label="Code Quality" value={scoreboard?.breakdown?.codeQuality || 0} max={10} color="blue" />
                      <ScoreBar label="Task Completion" value={scoreboard?.breakdown?.taskCompletion || 0} max={10} color="indigo" />
                      <ScoreBar label="Collaboration" value={scoreboard?.breakdown?.collaboration || 0} max={10} color="purple" />
                      <ScoreBar label="Innovation" value={scoreboard?.breakdown?.innovation || 0} max={10} color="amber" />
                   </div>
                </div>
             </div>

             <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200/60 shadow-sm shadow-blue-500/5">
                <h2 className="text-xl font-black tracking-tight text-slate-900 mb-8">Core Performance Indicators</h2>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   <KPICard icon={CheckSquare} color="blue" label="Total Tasks" value={kpis?.tasksCompleted || 0} />
                   <KPICard icon={Timer} color="emerald" label="Avg Speed" value={kpis?.onTimeDelivery || 'N/A'} />
                   <KPICard icon={AlertCircle} color="red" label="Bug Reports" value={kpis?.bugResolution || 0} />
                   <KPICard icon={FileText} color="purple" label="Reviews" value={kpis?.codeReviews || 0} />
                </div>
             </div>
          </div>
             ...
             <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200/60 shadow-sm shadow-blue-500/5">
                ...
                <div className="mb-10">
                   <div className="flex justify-between items-end mb-3">
                      ...
                      <span className="text-2xl font-black text-slate-900">{progress?.overall || 0}%</span>
                   </div>
                   <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-600 rounded-full transition-all duration-1000" style={{ width: `${progress?.overall || 0}%` }}></div>
                   </div>
                </div>
                ...
                <button onClick={() => window.location.href='/my-projects'} className="w-full mt-10 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all shadow-lg">Go to Project Hub</button>
             </div>
           ...
           <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {(leaderboardData || []).map((emp, i) => (
                 <div key={i} className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${
                    (emp._id || emp.id) === (user?._id || user?.id) ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100'
                 }`}>
...
const MetricCard = ({ icon: Icon, label, value, sub, color, progress, alert, onClick, clickable }) => (
  <div onClick={onClick} className={`bg-white p-8 rounded-[2.5rem] border border-slate-200/60 shadow-sm transition-all relative overflow-hidden group ${clickable ? 'cursor-pointer hover:border-blue-200 hover:shadow-xl' : ''}`}>
    ...
    {progress && <div className="mt-6 h-1 w-full bg-slate-50 rounded-full overflow-hidden"><div className="h-full bg-purple-500" style={{ width: `${progress}%` }}></div></div>}
  </div>
);
